...
static CachedDir* cached_dir_add_subdir(CachedDir* dir, const char* basename,
                                        const char* path);
...
static void clear_cache(CachedDir* dir, gpointer* cache) { *cache = NULL; }
...
static CachedDir* cached_dir_new_full(const char* name, GFunc notify,
                                      gpointer notify_data) {
  CachedDir* dir;
...
static CachedDir* cached_dir_ref(CachedDir* dir) {
  dir->references++;
  return dir;
}
...
static void cached_dir_unref(CachedDir* dir) {
  if (--dir->references == 0) {
    ...
    if (parent != NULL) cached_dir_remove_subdir(parent, dir->name);

    if (dir->notify) dir->notify(dir, dir->notify_data);
...
static inline CachedDir* find_subdir(CachedDir* dir, const char* subdir) {
  ...
  while (tmp != NULL) {
    ...
    if (strcmp(sub->name, subdir) == 0) return sub;
...
static DesktopEntry* cached_dir_find_relative_path(CachedDir* dir,
  ...
  while (split[i] != NULL) {
    if (split[i + 1] != NULL) {
      if ((dir = find_subdir(dir, split[i])) == NULL) break;
    } else {
...
static CachedDir* cached_dir_lookup(const char* canonical) {
  ...
  if (dir_cache == NULL)
    dir_cache = cached_dir_new_full("/", (GFunc)clear_cache, &dir_cache);
  dir = dir_cache;
...
static gboolean cached_dir_add_entry(CachedDir* dir, const char* basename,
                                     const char* path) {
  DesktopEntry* entry;
...
static gboolean cached_dir_update_entry(CachedDir* dir, const char* basename,
                                        const char* path) {
  GSList* tmp;
...
static gboolean emit_monitors_in_idle(void) {
...
static void cached_dir_queue_monitor_event(CachedDir* dir) {
  GSList* tmp;
  ...
  while (tmp != NULL) {
    ...
    if (dir->parent == d->parent && g_strcmp0(dir->name, d->name) == 0) break;
  ...
  if (monitors_idle_handler == 0) {
    monitors_idle_handler =
        g_idle_add((GSourceFunc)emit_monitors_in_idle, NULL);
  }
}
...
static gboolean cached_dir_load_entries_recursive(CachedDir* dir,
  ...
  if (dir->have_read_entries) return TRUE;
...
static void cached_dir_add_monitor(CachedDir* dir, EntryDirectory* ed,
                                   EntryDirectoryChangedFunc callback,
                                   gpointer user_data) {
  ...
  while (tmp != NULL) {
    ...
    if (monitor->ed == ed && monitor->callback == callback &&
...
static EntryDirectory* entry_directory_new_full(DesktopEntryType entry_type,
                                                const char* path,
                                                gboolean is_legacy,
...
static void entry_directory_add_monitor(EntryDirectory* ed,
                                        EntryDirectoryChangedFunc callback,
                                        gpointer user_data) {
  cached_dir_add_monitor(ed->dir, ed, callback, user_data);
}

static void entry_directory_remove_monitor(EntryDirectory* ed,
                                           EntryDirectoryChangedFunc callback,
                                           gpointer user_data) {
  cached_dir_remove_monitor(ed->dir, ed, callback, user_data);
}

static DesktopEntry* entry_directory_get_directory(EntryDirectory* ed,
                                                   const char* relative_path) {
  DesktopEntry* entry;

  if (ed->entry_type != DESKTOP_ENTRY_DIRECTORY) return NULL;
...
static char* get_desktop_file_id_from_path(EntryDirectory* ed,
...
static gboolean entry_directory_foreach_recursive(
    EntryDirectory* ed, CachedDir* cd, GString* relative_path,
    EntryDirectoryForeachFunc func, DesktopEntrySet* set, gpointer user_data) {
  GSList* tmp;
  gsize relative_path_len;

  if (cd->deleted) return TRUE;
  ...
  while (tmp != NULL) {
    ...
    if (desktop_entry_get_type(entry) == ed->entry_type) {
      ...
      g_string_append(relative_path, desktop_entry_get_basename(entry));

      file_id =
          get_desktop_file_id_from_path(ed, ed->entry_type, relative_path->str);
  ...
  while (tmp != NULL) {
    ...
    if (!entry_directory_foreach_recursive(ed, subdir, relative_path, func, set,
                                           user_data))
      return FALSE;
...
static void entry_directory_foreach(EntryDirectory* ed,
                                    EntryDirectoryForeachFunc func,
                                    DesktopEntrySet* set, gpointer user_data) {
  GString* path;

  path = g_string_new(NULL);

  entry_directory_foreach_recursive(ed, ed->dir, path, func, set, user_data);
...
void entry_directory_get_flat_contents(EntryDirectory* ed,
  ...
  while (tmp != NULL) {
    ...
    if (desktop_entries &&
      ...
      file_id =
          get_desktop_file_id_from_path(ed, DESKTOP_ENTRY_DESKTOP, basename);

      desktop_entry_set_add_entry(desktop_entries, entry, file_id);
    ...
    if (directory_entries &&
      ...
      desktop_entry_set_add_entry(directory_entries, entry, basename);
    }
  ...
  if (subdirs) *subdirs = g_slist_reverse(*subdirs);
}
...
void entry_directory_list_prepend(EntryDirectoryList* list,
                                  EntryDirectory* ed) {
  list->length += 1;
  list->dirs = g_list_prepend(list->dirs, entry_directory_ref(ed));
}
...
void entry_directory_list_append_list(EntryDirectoryList* list,
  ...
  while (tmp != NULL) {
    list->length += 1;
    new_dirs = g_list_prepend(new_dirs, entry_directory_ref(tmp->data));
...
DesktopEntry* entry_directory_list_get_directory(EntryDirectoryList* list,
  ...
  tmp = list->dirs;
  while (tmp != NULL) {
    if ((retval = entry_directory_get_directory(tmp->data, relative_path)) !=
        NULL)
      break;
...
static gboolean get_all_func(EntryDirectory* ed, DesktopEntry* entry,
                             const char* file_id, DesktopEntrySet* set,
                             gpointer user_data) {
...
DesktopEntrySet* _entry_directory_list_get_all_desktops(
    EntryDirectoryList* list) {
  GList* tmp;
  DesktopEntrySet* set;
