...
static inline MenuLayoutNode *node_next(MenuLayoutNode *node) {
  /* root nodes (no parent) never have siblings */
  if (node->parent == NULL) return NULL;

  /* circular list */
  if (node->next == node->parent->children) return NULL;

  return node->next;
}

static gboolean menu_layout_invoke_monitors(MenuLayoutNodeRoot *nr) {
  GSList *tmp;
...
static void handle_entry_directory_changed(EntryDirectory *dir,
                                           MenuLayoutNode *node) {
...
static void remove_entry_directory_list(MenuLayoutNodeMenu *nm,
                                        EntryDirectoryList **dirs) {
  if (*dirs) {
    entry_directory_list_remove_monitors(
        *dirs, (EntryDirectoryChangedFunc)handle_entry_directory_changed, nm);
...
void menu_layout_node_unref(MenuLayoutNode *node) {
  ...
  if (node->refcount == 0) {
    ...
    if (node->type == MENU_LAYOUT_NODE_MENU) {
      ...
      if (nm->name_node) menu_layout_node_unref(nm->name_node);
      ...
      if (nr->main_context != NULL) g_main_context_unref(nr->main_context);
...
#define RETURN_IF_NO_PARENT(node)                        \
  G_STMT_START {                                         \
    if ((node)->parent == NULL) {                        \
      g_warning(                                         \
          "To add siblings to a menu node, "             \
...
#define RETURN_IF_HAS_ENTRY_DIRS(node)                        \
  G_STMT_START {                                              \
    if ((node)->type == MENU_LAYOUT_NODE_MENU &&              \
      ...
      g_warning(                                              \
          "node acquired ->app_dirs or ->dir_dirs "           \
...
void menu_layout_node_insert_before(MenuLayoutNode *node,
  ...
  if (node == node->parent->children) node->parent->children = new_sibling;
...
static void recursive_clean_entry_directory_lists(MenuLayoutNode *node,
                                                  gboolean apps) {
  EntryDirectoryList **dirs;
  MenuLayoutNodeMenu *nm;
  MenuLayoutNode *iter;

  if (node->type != MENU_LAYOUT_NODE_MENU) return;
...
const char *menu_layout_node_menu_get_name(MenuLayoutNode *node) {
  ...
  if (nm->name_node == NULL) return NULL;

  return menu_layout_node_get_content(nm->name_node);
}

static void ensure_dir_lists(MenuLayoutNodeMenu *nm) {
  MenuLayoutNode *node;
  MenuLayoutNode *iter;
  EntryDirectoryList *app_dirs;
  EntryDirectoryList *dir_dirs;
  ...
  if (nm->app_dirs && nm->dir_dirs) return;
  ...
  while (iter != NULL) {
    ...
    if (iter->type == MENU_LAYOUT_NODE_LEGACY_DIR) {
      ...
      {
        ed = entry_directory_new_legacy(DESKTOP_ENTRY_DESKTOP, path,
                                        legacy->prefix);
        if (ed != NULL) {
          entry_directory_list_prepend(app_dirs, ed);
          entry_directory_unref(ed);
      ...
      {
        ed = entry_directory_new_legacy(DESKTOP_ENTRY_DIRECTORY, path,
                                        legacy->prefix);
        if (ed != NULL) {
          entry_directory_list_prepend(dir_dirs, ed);
          entry_directory_unref(ed);
  ...
  if (app_dirs) {
    ...
    entry_directory_list_add_monitors(
        nm->app_dirs, (EntryDirectoryChangedFunc)handle_entry_directory_changed,
        nm);
  }
  ...
  if (dir_dirs) {
    ...
    entry_directory_list_add_monitors(
        nm->dir_dirs, (EntryDirectoryChangedFunc)handle_entry_directory_changed,
        nm);
  }
}
...
EntryDirectoryList *menu_layout_node_menu_get_directory_dirs(
    MenuLayoutNode *node) {
  MenuLayoutNodeMenu *nm;
...
const char *menu_layout_node_move_get_old(MenuLayoutNode *node) {
  ...
  iter = node->children;
  while (iter != NULL) {
    if (iter->type == MENU_LAYOUT_NODE_OLD) return iter->content;
...
const char *menu_layout_node_move_get_new(MenuLayoutNode *node) {
  ...
  iter = node->children;
  while (iter != NULL) {
    if (iter->type == MENU_LAYOUT_NODE_NEW) return iter->content;
...
static void menu_layout_node_merge_set_type(MenuLayoutNode *node,
...
static void menu_layout_values_set(MenuLayoutValues *values,
...
static void menu_layout_node_default_layout_set_values(
    MenuLayoutNode *node, const char *show_empty, const char *inline_menus,
    const char *inline_limit, const char *inline_header,
    const char *inline_alias) {
  MenuLayoutNodeDefaultLayout *default_layout;
  ...
  menu_layout_values_set(&default_layout->layout_values, show_empty,
                         inline_menus, inline_limit, inline_header,
                         inline_alias);
}

static void menu_layout_node_menuname_set_values(MenuLayoutNode *node,
                                                 const char *show_empty,
                                                 const char *inline_menus,
                                                 const char *inline_limit,
                                                 const char *inline_header,
  ...
  menu_layout_values_set(&menuname->layout_values, show_empty, inline_menus,
                         inline_limit, inline_header, inline_alias);
}

void menu_layout_node_root_add_entries_monitor(
  ...
  while (tmp != NULL) {
    ...
    if (monitor->callback == callback && monitor->user_data == user_data) break;
...
void menu_layout_node_root_remove_entries_monitor(
  ...
  while (tmp != NULL) {
    ...
    if (monitor->callback == callback && monitor->user_data == user_data) {
...
static void set_error(GError **err, GMarkupParseContext *context,
                      GQuark error_domain, int error_code, const char *format,
                      ...) G_GNUC_PRINTF(5, 6);

static void add_context_to_error(GError **err, GMarkupParseContext *context);
...
static void end_element_handler(GMarkupParseContext *context,
                                ...
                                GError **error);
static void text_handler(GMarkupParseContext *context, const char *text,
                         gsize text_len, gpointer user_data, GError **error);
static void passthrough_handler(GMarkupParseContext *context,
                                const char *passthrough_text, gsize text_len,
                                gpointer user_data, GError **error);

static GMarkupParser menu_funcs = {start_element_handler, end_element_handler,
                                   text_handler, passthrough_handler, NULL};
...
static void set_error(GError **err, GMarkupParseContext *context,
  ...
  g_set_error(err, error_domain, error_code, "Line %d character %d: %s", line,
              ch, str);
...
static void add_context_to_error(GError **err, GMarkupParseContext *context) {
  ...
  str = g_strdup_printf("Line %d character %d: %s", line, ch, (*err)->message);
  g_free((*err)->message);
  (*err)->message = str;
}
...
static int has_child_of_type(MenuLayoutNode *node, MenuLayoutNodeType type) {
  MenuLayoutNode *iter;
...
static void push_node(MenuParser *parser, MenuLayoutNodeType type) {
  MenuLayoutNode *node;
...
static void start_menu_element(MenuParser *parser, GMarkupParseContext *context,
                               const char *element_name,
                               const char **attribute_names,
                               const char **attribute_values, GError **error) {
...
static void start_menu_child_element(MenuParser *parser,
  ...
  if (ELEMENT_IS("LegacyDir")) {
    ...
    if (!locate_attributes(context, element_name, attribute_names,
                           attribute_values, error, "prefix", &prefix, NULL))
      return;
    ...
    if (!locate_attributes(context, element_name, attribute_names,
                           attribute_values, error, "type", &type, NULL))
      return;
    ...
    locate_attributes(context, element_name, attribute_names, attribute_values,
                      error, "show_empty", &show_empty, "inline", &inline_menus,
                      "inline_limit", &inline_limit, "inline_header",
                      &inline_header, "inline_alias", &inline_alias, NULL);

    menu_layout_node_default_layout_set_values(parser->stack_top, show_empty,
                                               inline_menus, inline_limit,
                                               inline_header, inline_alias);
  } else {
    if (!check_no_attributes(context, element_name, attribute_names,
                             attribute_values, error))
      return;
    ...
    if (ELEMENT_IS("AppDir")) {
      ...
      if (has_child_of_type(parser->stack_top, MENU_LAYOUT_NODE_NAME)) {
        set_error(
            error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
      ...
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ELEMENT,
...
static void start_matching_rule_element(MenuParser *parser,
                                        GMarkupParseContext *context,
                                        const char *element_name,
                                        const char **attribute_names,
                                        const char **attribute_values,
...
static void start_move_child_element(MenuParser *parser,
                                     GMarkupParseContext *context,
                                     const char *element_name,
                                     const char **attribute_names,
                                     const char **attribute_values,
...
static void start_layout_child_element(MenuParser *parser,
                                       GMarkupParseContext *context,
                                       const char *element_name,
                                       const char **attribute_names,
                                       const char **attribute_values,
  ...
  if (ELEMENT_IS("Menuname")) {
    ...
    menu_layout_node_menuname_set_values(parser->stack_top, show_empty,
                                         inline_menus, inline_limit,
                                         inline_header, inline_alias);
...
static void start_element_handler(GMarkupParseContext *context,
  ...
  if (ELEMENT_IS("Menu")) {
    if (parser->stack_top == parser->root &&
      ...
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                "Multiple root elements in menu file, only one toplevel <Menu> "
                "is allowed\n");
      return;
    }

    start_menu_element(parser, context, element_name, attribute_names,
                       attribute_values, error);
    ...
    start_menu_child_element(parser, context, element_name, attribute_names,
                             attribute_values, error);
    ...
    start_matching_rule_element(parser, context, element_name, attribute_names,
                                attribute_values, error);
  } else if (parser->stack_top->type == MENU_LAYOUT_NODE_MOVE) {
    start_move_child_element(parser, context, element_name, attribute_names,
                             attribute_values, error);
  } else if (parser->stack_top->type == MENU_LAYOUT_NODE_LAYOUT ||
             parser->stack_top->type == MENU_LAYOUT_NODE_DEFAULT_LAYOUT) {
    start_layout_child_element(parser, context, element_name, attribute_names,
                               attribute_values, error);
  } else {
    set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_UNKNOWN_ELEMENT,
              "Element <%s> may not appear in this context\n", element_name);
 ...
 */
static gboolean fixup_layout_node(GMarkupParseContext *context,
                                  MenuParser *parser, MenuLayoutNode *node,
                                  GError **error) {
 ...
 */
static gboolean fixup_move_node(GMarkupParseContext *context,
                                MenuParser *parser, MenuLayoutNode *node,
                                GError **error) {
  MenuLayoutNode *child;
  int n_old;
...
static void end_element_handler(GMarkupParseContext *context,
  ...
  switch (parser->stack_top->type) {
    ...
    case MENU_LAYOUT_NODE_MOVE:
      if (!fixup_move_node(context, parser, parser->stack_top, error)) goto out;
      break;
  }
...
static gboolean all_whitespace(const char *text, gsize text_len) {
  const char *p;
  const char *end;
...
static void menu_parser_init(MenuParser *parser) {
  parser->root = menu_layout_node_new(MENU_LAYOUT_NODE_ROOT);
  parser->stack_top = parser->root;
}

static void menu_parser_free(MenuParser *parser) {
  if (parser->root) menu_layout_node_unref(parser->root);
}
...
out:
  ...
  if (error) {
    ...
    g_set_error(err, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                "Menu file %s did not contain a root <Menu> element", filename);
  }
