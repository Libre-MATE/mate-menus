 ...
 */
static const char *unix_basename_from_path(const char *path) {
...
static const char *get_current_desktop(void) {
  ...
  if (g_strcmp0(current_desktop, "*") == 0) return NULL;

  return current_desktop;
}

static GIcon *key_file_get_icon(GKeyFile *key_file) {
  GIcon *icon = NULL;
  gchar *icon_name;
...
static gboolean key_file_get_show_in(GKeyFile *key_file) {
  ...
  if (!current_desktop) return TRUE;

  strv = g_key_file_get_string_list(key_file, DESKTOP_ENTRY_GROUP, "OnlyShowIn",
                                    NULL, NULL);
  if (strv) {
    show_in = FALSE;
    for (i = 0; strv[i]; i++) {
    ...
    strv = g_key_file_get_string_list(key_file, DESKTOP_ENTRY_GROUP,
                                      "NotShowIn", NULL, NULL);
    if (strv) {
      show_in = TRUE;
      for (i = 0; strv[i]; i++) {
...
static gboolean desktop_entry_load_directory(DesktopEntry *entry,
                                             GKeyFile *key_file,
                                             GError **error) {
  ...
  type_str =
      g_key_file_get_string(key_file, DESKTOP_ENTRY_GROUP, "Type", error);
  ...
  entry_directory->name = g_key_file_get_locale_string(
      key_file, DESKTOP_ENTRY_GROUP, "Name", NULL, error);
  if (entry_directory->name == NULL) return FALSE;

  entry_directory->generic_name = g_key_file_get_locale_string(
      key_file, DESKTOP_ENTRY_GROUP, "GenericName", NULL, NULL);
  entry_directory->comment = g_key_file_get_locale_string(
      key_file, DESKTOP_ENTRY_GROUP, "Comment", NULL, NULL);
  entry_directory->icon = key_file_get_icon(key_file);
  entry_directory->nodisplay =
      g_key_file_get_boolean(key_file, DESKTOP_ENTRY_GROUP, "NoDisplay",
                             NULL) != FALSE;
  entry_directory->hidden =
      g_key_file_get_boolean(key_file, DESKTOP_ENTRY_GROUP, "Hidden", NULL) !=
      FALSE;
...
static gboolean desktop_entry_load(DesktopEntry *entry) {
  if (entry->type == DESKTOP_ENTRY_DESKTOP) {
    ...
    if (!g_key_file_load_from_file(key_file, entry->path, 0, &error)) goto out;

    if (!desktop_entry_load_directory(entry, key_file, &error)) goto out;
...
void desktop_entry_unref(DesktopEntry *entry) {
  ...
  if (entry->type == DESKTOP_ENTRY_DESKTOP) {
    ...
    if (desktop_entry->appinfo) g_object_unref(desktop_entry->appinfo);
  } else if (entry->type == DESKTOP_ENTRY_DIRECTORY) {
    DesktopEntryDirectory *entry_directory = (DesktopEntryDirectory *)entry;
...
const char *desktop_entry_get_path(DesktopEntry *entry) { return entry->path; }
...
const char *desktop_entry_get_name(DesktopEntry *entry) {
  if (entry->type == DESKTOP_ENTRY_DESKTOP)
    return g_app_info_get_name(
        G_APP_INFO(((DesktopEntryDesktop *)entry)->appinfo));
...
const char *desktop_entry_get_comment(DesktopEntry *entry) {
  if (entry->type == DESKTOP_ENTRY_DESKTOP)
    return g_app_info_get_description(
        G_APP_INFO(((DesktopEntryDesktop *)entry)->appinfo));
...
gboolean desktop_entry_has_categories(DesktopEntry *entry) {
  ...
  if (entry->type != DESKTOP_ENTRY_DESKTOP) return FALSE;

  desktop_entry = (DesktopEntryDesktop *)entry;
  return (desktop_entry->categories != NULL &&
          desktop_entry->categories[0] != 0);
}
...
gboolean desktop_entry_has_category(DesktopEntry *entry, const char *category) {
  ...
  if (desktop_entry->categories == NULL) return FALSE;

  if (!(quark = g_quark_try_string(category))) return FALSE;

  for (i = 0; desktop_entry->categories[i]; i++) {
    if (quark == desktop_entry->categories[i]) return TRUE;
  }
...
void desktop_entry_set_unref(DesktopEntrySet *set) {
  ...
  if (set->refcount == 0) {
    ...
    if (set->hash) g_hash_table_destroy(set->hash);
    set->hash = NULL;
...
void desktop_entry_set_add_entry(DesktopEntrySet *set, DesktopEntry *entry,
                                 const char *file_id) {
  menu_verbose(" Adding to set %p entry %s\n", set, file_id);

  if (set->hash == NULL) {
    set->hash = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
                                      (GDestroyNotify)desktop_entry_unref);
  }

  g_hash_table_replace(set->hash, g_strdup(file_id), desktop_entry_ref(entry));
}

DesktopEntry *desktop_entry_set_lookup(DesktopEntrySet *set,
                                       const char *file_id) {
  if (set->hash == NULL) return NULL;
...
static void entry_hash_foreach(const char *file_id, DesktopEntry *entry,
...
void desktop_entry_set_foreach(DesktopEntrySet *set,
  ...
  if (set->hash != NULL) {
    ...
    g_hash_table_foreach(set->hash, (GHFunc)entry_hash_foreach, &fd);
  }
}
...
guint desktop_entry_set_get_count(DesktopEntrySet *set) {
  if (set->hash == NULL) return 0;

  return g_hash_table_size(set->hash);
}

static void union_foreach(const char *file_id, DesktopEntry *entry,
                          DesktopEntrySet *set) {
...
void desktop_entry_set_union(DesktopEntrySet *set, DesktopEntrySet *with) {
  ...
  if (desktop_entry_set_get_count(with) == 0) return; /* A fast simple case */

  g_hash_table_foreach(with->hash, (GHFunc)union_foreach, set);
}
...
static gboolean intersect_foreach_remove(const char *file_id,
  ...
  if (g_hash_table_lookup(id->with->hash, file_id) != NULL) return FALSE;
...
void desktop_entry_set_intersection(DesktopEntrySet *set,
  ...
  g_hash_table_foreach_remove(set->hash, (GHRFunc)intersect_foreach_remove,
                              &id);
}
...
static gboolean subtract_foreach_remove(const char *file_id,
                                        DesktopEntry *entry, SubtractData *sd) {
  /* Remove everything in "set" which is not in "other" */

  if (g_hash_table_lookup(sd->other->hash, file_id) == NULL) return FALSE;
...
void desktop_entry_set_subtract(DesktopEntrySet *set, DesktopEntrySet *other) {
  ...
  g_hash_table_foreach_remove(set->hash, (GHRFunc)subtract_foreach_remove, &sd);
}
