...
static void append_to_string(MenuLayoutNode* node, gboolean onelevel, int depth,
                             GString* str);
...
static void append_simple_with_attr(MenuLayoutNode* node, int depth,
                                    const char* node_name,
                                    const char* attr_name,
                                    const char* attr_value, GString* str) {
  const char* content;
  ...
  if ((content = menu_layout_node_get_content(node))) {
    ...
    if (attr_name && attr_value) {
      ...
      g_string_append_printf(str, "<%s %s=\"%s\">%s</%s>\n", node_name,
                             attr_name, attr_escaped, escaped, node_name);
...
static void append_layout(MenuLayoutNode* node, int depth,
                          const char* node_name,
                          MenuLayoutValues* layout_values, GString* str) {
  const char* content;
...
static void append_merge(MenuLayoutNode* node, int depth, const char* node_name,
                         MenuLayoutMergeType merge_type, GString* str) {
...
static void append_simple(MenuLayoutNode* node, int depth,
                          const char* node_name, GString* str) {
  append_simple_with_attr(node, depth, node_name, NULL, NULL, str);
}

static void append_start(MenuLayoutNode* node, int depth, const char* node_name,
                         GString* str) {
  append_spaces(str, depth);
...
static void append_end(MenuLayoutNode* node, int depth, const char* node_name,
                       GString* str) {
  append_spaces(str, depth);
...
static void append_container(MenuLayoutNode* node, gboolean onelevel, int depth,
                             const char* node_name, GString* str) {
...
static void append_to_string(MenuLayoutNode* node, gboolean onelevel, int depth,
  ...
  switch (menu_layout_node_get_type(node)) {
    ...
    case MENU_LAYOUT_NODE_MERGE_FILE: {
      ...
      append_simple_with_attr(
          node, depth, "MergeFile", "type",
          type == MENU_MERGE_FILE_TYPE_PARENT ? "parent" : "path", str);
      break;
    }
    ...
    case MENU_LAYOUT_NODE_LEGACY_DIR:
      append_simple_with_attr(node, depth, "LegacyDir", "prefix",
                              menu_layout_node_legacy_dir_get_prefix(node),
                              str);
      break;
    ...
    case MENU_LAYOUT_NODE_MERGE:
      append_merge(node, depth, "Merge", menu_layout_node_merge_get_type(node),
                   str);
      break;
