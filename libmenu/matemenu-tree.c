...
static gboolean matemenu_tree_load_layout(MateMenuTree *tree, GError **error);
...
#ifdef WITH_COLLECTION
static void collection_applet_changed(GSettings *settings, gchar *key,
                                      MateMenuTree *self);
#endif /* WITH_COLLECTION */
...
static void handle_nonexistent_menu_file_changed(MenuMonitor *monitor,
...
static void handle_menu_file_changed(MenuMonitor *monitor,
                                     MenuMonitorEvent event, const char *path,
                                     MateMenuTree *tree) {
  menu_verbose("\"%s\" %s, marking tree for recanicalization\n", path,
               event == MENU_MONITOR_EVENT_CREATED   ? "created"
               : event == MENU_MONITOR_EVENT_CHANGED ? "changed"
                                                     : "deleted");
...
static void handle_menu_file_directory_changed(MenuMonitor *monitor,
                                               MenuMonitorEvent event,
                                               const char *path,
                                               MateMenuTree *tree) {
  if (!g_str_has_suffix(path, ".menu")) return;
...
static void matemenu_tree_add_menu_file_monitor(MateMenuTree *tree,
                                                const char *path,
                                                MenuFileMonitorType type) {
  ...
  switch (type) {
    ...
    case MENU_FILE_MONITOR_NONEXISTENT_FILE:
      ...
      menu_monitor_add_notify(
          monitor->monitor,
          (MenuMonitorNotifyFunc)handle_nonexistent_menu_file_changed, tree);
      break;
    ...
    case MENU_FILE_MONITOR_DIRECTORY:
      ...
      menu_monitor_add_notify(
          monitor->monitor,
          (MenuMonitorNotifyFunc)handle_menu_file_directory_changed, tree);
      break;
...
static void remove_menu_file_monitor(MenuFileMonitor *monitor,
                                     MateMenuTree *tree) {
  switch (monitor->type) {
    case MENU_FILE_MONITOR_FILE:
      menu_monitor_remove_notify(
          monitor->monitor, (MenuMonitorNotifyFunc)handle_menu_file_changed,
          tree);
      break;
...
static void matemenu_tree_remove_menu_file_monitors(MateMenuTree *tree) {
  menu_verbose("Removing all menu file monitors\n");

  g_slist_foreach(tree->menu_file_monitors, (GFunc)remove_menu_file_monitor,
                  tree);
  g_slist_free(tree->menu_file_monitors);
  tree->menu_file_monitors = NULL;
}

static gboolean canonicalize_path(MateMenuTree *tree, const char *path) {
  tree->canonical_path = realpath(path, NULL);
  if (tree->canonical_path) {
    tree->canonical = TRUE;
    matemenu_tree_add_menu_file_monitor(tree, tree->canonical_path,
                                        MENU_FILE_MONITOR_FILE);
  } else {
    matemenu_tree_add_menu_file_monitor(tree, path,
                                        MENU_FILE_MONITOR_NONEXISTENT_FILE);
  }
...
static gboolean canonicalize_basename_with_config_dir(MateMenuTree *tree,
                                                      const char *basename,
...
static void canonicalize_basename(MateMenuTree *tree, const char *basename) {
  if (!canonicalize_basename_with_config_dir(tree, basename,
...
static gboolean matemenu_tree_canonicalize_path(MateMenuTree *tree,
  ...
  if (tree->path) {
    ...
    if (xdg_menu_prefix != NULL) {
      ...
      prefixed_basename =
          g_strdup_printf("%sapplications.menu", xdg_menu_prefix);
    ...
    if (!tree->canonical) canonicalize_basename(tree, tree->basename);
  }
...
MateMenuTree *matemenu_tree_new(const char *menu_basename,
  ...
  return g_object_new(MATEMENU_TYPE_TREE, "menu-basename", menu_basename,
                      "flags", flags, NULL);
}
...
MateMenuTree *matemenu_tree_new_for_path(const char *menu_path,
  ...
  return g_object_new(MATEMENU_TYPE_TREE, "menu-path", menu_path, "flags",
                      flags, NULL);
}

static GObject *matemenu_tree_constructor(
    GType type, guint n_construct_properties,
    GObjectConstructParam *construct_properties) {
  GObject *obj;
  MateMenuTree *self;

  obj = G_OBJECT_CLASS(matemenu_tree_parent_class)
  ...
  if (self->path != NULL) g_object_set(self, "menu-basename", NULL, NULL);

  return obj;
}

static void matemenu_tree_set_property(GObject *object, guint prop_id,
...
static void matemenu_tree_get_property(GObject *object, guint prop_id,
...
static void matemenu_tree_finalize(GObject *object) {
  MateMenuTree *tree = MATEMENU_TREE(object);

  matemenu_tree_force_recanonicalize(tree);

  if (tree->basename != NULL) g_free(tree->basename);
  tree->basename = NULL;

  g_free(tree->non_prefixed_basename);
  tree->non_prefixed_basename = NULL;

  if (tree->path != NULL) g_free(tree->path);
  tree->path = NULL;

  if (tree->canonical_path != NULL) g_free(tree->canonical_path);
  tree->canonical_path = NULL;
  ...
  g_signal_handlers_disconnect_by_func(
      tree->settings, G_CALLBACK(collection_applet_changed), tree);
...
static void matemenu_tree_init(MateMenuTree *self) {
  ...
  g_signal_connect(self->settings, "changed::object-id-list",
                   G_CALLBACK(collection_applet_changed), self);
#endif /* WITH_COLLECTION */
}

static void matemenu_tree_class_init(MateMenuTreeClass *klass) {
  ...
  g_object_class_install_property(
      gobject_class, PROP_MENU_BASENAME,
      g_param_spec_string("menu-basename", "", "", "applications.menu",
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("menu-path", "", "", NULL,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
  /**
   * MateMenuTree:flags:
   *
  ...
  g_object_class_install_property(
      ...
      g_param_spec_flags("flags", "", "", MATEMENU_TYPE_TREE_FLAGS,
 ...
 */
gboolean matemenu_tree_load_sync(MateMenuTree *tree, GError **error) {
...
static MateMenuTreeDirectory *find_path(MateMenuTreeDirectory *directory,
                                        const char *path) {
  const char *name;
  char *slash;
  char *freeme;
...
MateMenuTreeDirectory *matemenu_tree_get_directory_from_path(MateMenuTree *tree,
  ...
  if (path[0] != G_DIR_SEPARATOR) return NULL;

  if (!(root = matemenu_tree_get_root_directory(tree))) return NULL;
 ...
 *
 * Returns: (transfer full): A newly referenced #MateMenuTreeEntry, or %NULL if
 * none
 */
MateMenuTreeEntry *matemenu_tree_get_entry_by_id(MateMenuTree *tree,
  ...
  if (entry != NULL) matemenu_tree_item_ref(entry);
...
static MateMenuTreeDirectory *get_parent(MateMenuTreeItem *item) {
 ...
 */
MateMenuTreeDirectory *matemenu_tree_directory_get_parent(
    MateMenuTreeDirectory *directory) {
 ...
 * Returns: (transfer full): The parent directory, or %NULL if none
 */
MateMenuTreeDirectory *matemenu_tree_entry_get_parent(
    MateMenuTreeEntry *entry) {
 ...
 * Returns: (transfer full): The parent directory, or %NULL if none
 */
MateMenuTreeDirectory *matemenu_tree_alias_get_parent(
 ...
 * Returns: (transfer full): The parent directory, or %NULL if none
 */
MateMenuTreeDirectory *matemenu_tree_header_get_parent(
 ...
 * Returns: (transfer full): The parent directory, or %NULL if none
 */
MateMenuTreeDirectory *matemenu_tree_separator_get_parent(
    MateMenuTreeSeparator *separator) {
  return get_parent((MateMenuTreeItem *)separator);
}

static void matemenu_tree_item_set_parent(MateMenuTreeItem *item,
                                          MateMenuTreeDirectory *parent) {
 ...
 */
MateMenuTreeIter *matemenu_tree_directory_iter(
    MateMenuTreeDirectory *directory) {
 ...
 */
MateMenuTreeDirectory *matemenu_tree_iter_get_directory(
    MateMenuTreeIter *iter) {
 ...
 */
MateMenuTreeSeparator *matemenu_tree_iter_get_separator(
    MateMenuTreeIter *iter) {
  g_return_val_if_fail(iter != NULL, NULL);
  g_return_val_if_fail(iter->item != NULL, NULL);
...
const char *matemenu_tree_directory_get_name(MateMenuTreeDirectory *directory) {
  ...
  if (!directory->directory_entry) return directory->name;

  return desktop_entry_get_name(directory->directory_entry);
}

const char *matemenu_tree_directory_get_generic_name(
    MateMenuTreeDirectory *directory) {
  g_return_val_if_fail(directory != NULL, NULL);

  if (!directory->directory_entry) return NULL;

  return desktop_entry_get_generic_name(directory->directory_entry);
}

const char *matemenu_tree_directory_get_comment(
    MateMenuTreeDirectory *directory) {
  g_return_val_if_fail(directory != NULL, NULL);
...
const char *matemenu_tree_directory_get_desktop_file_path(
    MateMenuTreeDirectory *directory) {
  g_return_val_if_fail(directory != NULL, NULL);
...
const char *matemenu_tree_directory_get_menu_id(
    MateMenuTreeDirectory *directory) {
  g_return_val_if_fail(directory != NULL, NULL);
...
gboolean matemenu_tree_directory_get_is_nodisplay(
    MateMenuTreeDirectory *directory) {
 ...
 */
MateMenuTree *matemenu_tree_directory_get_tree(
    MateMenuTreeDirectory *directory) {
  g_return_val_if_fail(directory != NULL, NULL);
...
const char *matemenu_tree_entry_get_desktop_file_path(
    MateMenuTreeEntry *entry) {
  g_return_val_if_fail(entry != NULL, NULL);
...
gboolean matemenu_tree_entry_get_is_nodisplay_recurse(
    MateMenuTreeEntry *entry) {
  MateMenuTreeDirectory *directory;
  ...
  if (g_desktop_app_info_get_nodisplay(app_info)) return TRUE;

  directory = entry->item.parent;
  while (directory != NULL) {
    if (directory->is_nodisplay) return TRUE;
...
MateMenuTreeDirectory *matemenu_tree_header_get_directory(
    MateMenuTreeHeader *header) {
...
MateMenuTreeItemType matemenu_tree_alias_get_aliased_item_type(
    MateMenuTreeAlias *alias) {
 ...
 * Returns: (transfer full): The #MateMenuTree
 */
MateMenuTree *matemenu_tree_separator_get_tree(
    MateMenuTreeSeparator *separator) {
 ...
 */
MateMenuTreeDirectory *matemenu_tree_alias_get_aliased_directory(
    MateMenuTreeAlias *alias) {
  g_return_val_if_fail(alias != NULL, NULL);
  g_return_val_if_fail(
      alias->aliased_item->type == MATEMENU_TREE_ITEM_DIRECTORY, NULL);
 ...
 */
MateMenuTreeEntry *matemenu_tree_alias_get_aliased_entry(
    MateMenuTreeAlias *alias) {
  g_return_val_if_fail(alias != NULL, NULL);
  g_return_val_if_fail(alias->aliased_item->type == MATEMENU_TREE_ITEM_ENTRY,
                       NULL);

  return (MateMenuTreeEntry *)matemenu_tree_item_ref(alias->aliased_item);
}

static MateMenuTreeDirectory *matemenu_tree_directory_new(
    MateMenuTree *tree, MateMenuTreeDirectory *parent, const char *name) {
...
static MateMenuTreeSeparator *matemenu_tree_separator_new(
    MateMenuTreeDirectory *parent) {
...
static void matemenu_tree_separator_finalize(MateMenuTreeSeparator *separator) {
  g_assert(separator->item.refcount == 0);

  g_slice_free(MateMenuTreeSeparator, separator);
}

static MateMenuTreeHeader *matemenu_tree_header_new(
    MateMenuTreeDirectory *parent, MateMenuTreeDirectory *directory) {
...
static void matemenu_tree_header_finalize(MateMenuTreeHeader *header) {
  g_assert(header->item.refcount == 0);

  if (header->directory != NULL) matemenu_tree_item_unref(header->directory);
  header->directory = NULL;

  g_slice_free(MateMenuTreeHeader, header);
}

static MateMenuTreeAlias *matemenu_tree_alias_new(
    MateMenuTreeDirectory *parent, MateMenuTreeDirectory *directory,
    MateMenuTreeItem *item) {
...
static void matemenu_tree_alias_finalize(MateMenuTreeAlias *alias) {
  g_assert(alias->item.refcount == 0);

  if (alias->directory != NULL) matemenu_tree_item_unref(alias->directory);
...
static MateMenuTreeEntry *matemenu_tree_entry_new(MateMenuTreeDirectory *parent,
                                                  DesktopEntry *desktop_entry,
...
static void matemenu_tree_entry_finalize(MateMenuTreeEntry *entry) {
  g_assert(entry->item.refcount == 0);

  g_free(entry->desktop_file_id);
  entry->desktop_file_id = NULL;

  if (entry->desktop_entry) desktop_entry_unref(entry->desktop_entry);
  entry->desktop_entry = NULL;

  g_slice_free(MateMenuTreeEntry, entry);
}

static int matemenu_tree_entry_compare_by_id(MateMenuTreeItem *a,
                                             MateMenuTreeItem *b) {
 ...
 *
 * Returns: (transfer full): The same @item, or %NULL if @item is not a valid
 * #MateMenuTreeItem
 */
...
void matemenu_tree_item_unref(gpointer itemp) {
  MateMenuTreeItem *item;
...
static inline const char *matemenu_tree_item_compare_get_name_helper(
    MateMenuTreeItem *item, MateMenuTreeFlags flags) {
  const char *name;
  ...
  switch (item->type) {
    case MATEMENU_TREE_ITEM_DIRECTORY:
      if (MATEMENU_TREE_DIRECTORY(item)->directory_entry)
        name = desktop_entry_get_name(
            MATEMENU_TREE_DIRECTORY(item)->directory_entry);
      else
    ...
    case MATEMENU_TREE_ITEM_ENTRY:
      if (flags & MATEMENU_TREE_FLAGS_SORT_DISPLAY_NAME)
        name = g_app_info_get_display_name(G_APP_INFO(
            matemenu_tree_entry_get_app_info(MATEMENU_TREE_ENTRY(item))));
      else
...
static int matemenu_tree_item_compare(MateMenuTreeItem *a, MateMenuTreeItem *b,
                                      gpointer flags_p) {
  const char *name_a;
  const char *name_b;
  MateMenuTreeFlags flags;
...
static MenuLayoutNode *find_menu_child(MenuLayoutNode *layout) {
...
static void merge_resolved_children(MateMenuTree *tree,
                                    GHashTable *loaded_menu_files,
...
static gboolean load_merge_file(MateMenuTree *tree,
                                GHashTable *loaded_menu_files,
                                const char *filename, gboolean is_canonical,
                                gboolean add_monitor, MenuLayoutNode *where) {
  ...
  if (!is_canonical) {
    ...
    if (canonical == NULL) {
      if (add_monitor)
        matemenu_tree_add_menu_file_monitor(tree, filename,
                                            MENU_FILE_MONITOR_NONEXISTENT_FILE);
  ...
  g_hash_table_insert(loaded_menu_files, (char *)canonical,
                      GUINT_TO_POINTER(TRUE));

  if (add_monitor)
    matemenu_tree_add_menu_file_monitor(tree, canonical,
                                        MENU_FILE_MONITOR_FILE);
...
static gboolean load_merge_file_with_config_dir(MateMenuTree *tree,
                                                GHashTable *loaded_menu_files,
...
static gboolean compare_basedir_to_config_dir(const char *canonical_basedir,
                                              const char *config_dir) {
  char *dirname;
...
static gboolean load_parent_merge_file_from_basename(
    MateMenuTree *tree, GHashTable *loaded_menu_files, MenuLayoutNode *layout,
    const char *menu_file, const char *canonical_basedir) {
  ...
  found_basedir =
      compare_basedir_to_config_dir(canonical_basedir, g_get_user_config_dir());
  ...
  while (system_config_dirs[i] != NULL) {
    if (!found_basedir) {
      ...
      if (load_merge_file_with_config_dir(tree, loaded_menu_files, menu_file,
                                          system_config_dirs[i], layout)) {
        break;
      }
    }
...
static gboolean load_parent_merge_file(MateMenuTree *tree,
                                       GHashTable *loaded_menu_files,
                                       MenuLayoutNode *layout) {
  MenuLayoutNode *root;
  const char *basedir;
  ...
  if (strcmp(menu_file, "mate-applications.menu") == 0 &&
    ...
    prefixed_basename =
        g_strdup_printf("%s%s", g_getenv("XDG_MENU_PREFIX"), menu_file);
    found = load_parent_merge_file_from_basename(
        tree, loaded_menu_files, layout, prefixed_basename, canonical_basedir);
    g_free(prefixed_basename);
  }

  if (!found) {
    found = load_parent_merge_file_from_basename(
        tree, loaded_menu_files, layout, menu_file, canonical_basedir);
  }
...
static void load_merge_dir(MateMenuTree *tree, GHashTable *loaded_menu_files,
                           const char *dirname, MenuLayoutNode *where) {
  GDir *dir;
  const char *menu_file;

  menu_verbose("Loading merge dir \"%s\"\n", dirname);

  matemenu_tree_add_menu_file_monitor(tree, dirname,
                                      MENU_FILE_MONITOR_DIRECTORY);

  if ((dir = g_dir_open(dirname, 0, NULL)) == NULL) return;
...
static void load_merge_dir_with_config_dir(MateMenuTree *tree,
                                           GHashTable *loaded_menu_files,
                                           const char *config_dir,
                                           const char *dirname,
                                           MenuLayoutNode *where) {
...
static void resolve_merge_file(MateMenuTree *tree,
                               GHashTable *loaded_menu_files,
                               MenuLayoutNode *layout) {
  char *filename;

  if (menu_layout_node_merge_file_get_type(layout) ==
      MENU_MERGE_FILE_TYPE_PARENT) {
    if (load_parent_merge_file(tree, loaded_menu_files, layout)) return;
  }
...
static void resolve_merge_dir(MateMenuTree *tree, GHashTable *loaded_menu_files,
                              MenuLayoutNode *layout) {
  char *path;
...
static MenuLayoutNode *add_app_dir(MateMenuTree *tree, MenuLayoutNode *before,
                                   const char *data_dir) {
  MenuLayoutNode *tmp;
  char *dirname;
...
static void resolve_default_app_dirs(MateMenuTree *tree,
                                     MenuLayoutNode *layout) {
  ...
  before =
      add_app_dir(tree, menu_layout_node_ref(layout), g_get_user_data_dir());
...
static MenuLayoutNode *add_directory_dir(MateMenuTree *tree,
                                         MenuLayoutNode *before,
                                         const char *data_dir) {
  MenuLayoutNode *tmp;
  char *dirname;
  ...
  menu_verbose(
      "Adding <DirectoryDir>%s</DirectoryDir> in <DefaultDirectoryDirs/>\n",
      dirname);
...
/* According to desktop spec, since our menu file is called 'mate-applications',
 * our merged menu folders need to be called 'mate-applications-merged'.  We'll
 * setup the folder 'applications-merged' if it doesn't exist yet, and a symlink
 * pointing to it in the
 * ~/.config/menus directory
 */
static void setup_merge_dir_symlink(void) {
  gchar *user_config = (gchar *)g_get_user_config_dir();
  gchar *merge_path =
      g_build_filename(user_config, "menus", "applications-merged", NULL);
  ...
  sym_path =
      g_build_filename(user_config, "menus", "mate-applications-merged", NULL);
...
static void resolve_default_directory_dirs(MateMenuTree *tree,
                                           MenuLayoutNode *layout) {
  MenuLayoutNode *before;
  const char *const *system_data_dirs;
  int i;
  ...
  before = add_directory_dir(tree, menu_layout_node_ref(layout),
                             g_get_user_data_dir());
...
static void resolve_default_merge_dirs(MateMenuTree *tree,
                                       GHashTable *loaded_menu_files,
                                       MenuLayoutNode *layout) {
  MenuLayoutNode *root;
  const char *menu_name;
  ...
  while (i > 0) {
    i--;
    load_merge_dir_with_config_dir(tree, loaded_menu_files,
                                   system_config_dirs[i], merge_name, layout);
  }

  load_merge_dir_with_config_dir(tree, loaded_menu_files,
                                 g_get_user_config_dir(), merge_name, layout);
...
static void add_filename_include(const char *desktop_file_id,
                                 DesktopEntry *entry, MenuLayoutNode *include) {
...
static void is_dot_directory(const char *basename, DesktopEntry *entry,
                             gboolean *has_dot_directory) {
  if (!strcmp(basename, ".directory")) *has_dot_directory = TRUE;
}

static gboolean add_menu_for_legacy_dir(MenuLayoutNode *parent,
  ...
  entry_directory_get_flat_contents(ed, desktop_entries, directory_entries,
                                    &subdirs);
  ...
  if (desktop_entry_set_get_count(desktop_entries) > 0 || subdirs) {
    ...
    if (desktop_entry_set_get_count(desktop_entries) > 0) {
      ...
      desktop_entry_set_foreach(
          desktop_entries, (DesktopEntrySetForeachFunc)add_filename_include,
          include);
    ...
    while (tmp != NULL) {
      ...
      add_menu_for_legacy_dir(menu, subdir_path->str, subdir_relative->str,
                              legacy_prefix, subdir);
...
static void resolve_legacy_dir(MateMenuTree *tree,
                               GHashTable *loaded_menu_files,
  ...
  if (add_menu_for_legacy_dir(to_merge, menu_layout_node_get_content(legacy),
                              NULL,
...
static MenuLayoutNode *add_legacy_dir(MateMenuTree *tree,
                                      GHashTable *loaded_menu_files,
                                      MenuLayoutNode *before,
                                      const char *data_dir) {
  MenuLayoutNode *legacy;
...
static void resolve_kde_legacy_dirs(MateMenuTree *tree,
                                    GHashTable *loaded_menu_files,
                                    MenuLayoutNode *layout) {
  MenuLayoutNode *before;
  const char *const *system_data_dirs;
  ...
  before = add_legacy_dir(tree, loaded_menu_files, menu_layout_node_ref(layout),
                          g_get_user_data_dir());

  i = 0;
  while (system_data_dirs[i] != NULL) {
    before =
        add_legacy_dir(tree, loaded_menu_files, before, system_data_dirs[i]);
...
static void move_children(MenuLayoutNode *from, MenuLayoutNode *to) {
  MenuLayoutNode *from_child;
...
static int null_safe_strcmp(const char *a, const char *b) {
  if (a == NULL && b == NULL)
    return 0;
  else if (a == NULL)
    return -1;
...
static int node_menu_compare_func(const void *a, const void *b) {
  MenuLayoutNode *node_a = (MenuLayoutNode *)a;
  MenuLayoutNode *node_b = (MenuLayoutNode *)b;
...
static void matemenu_tree_strip_duplicate_children(MateMenuTree *tree,
                                                   MenuLayoutNode *layout) {
  MenuLayoutNode *child;
  GSList *simple_nodes;
  ...
  simple_nodes = g_slist_sort(simple_nodes, node_compare_func);
   ...
   */
  menu_layout_nodes = g_slist_sort(menu_layout_nodes, node_menu_compare_func);
...
static MenuLayoutNode *find_submenu(MenuLayoutNode *layout, const char *path,
  ...
  if (path[0] == '\0' || path[0] == G_DIR_SEPARATOR) return NULL;

  slash = strchr(path, G_DIR_SEPARATOR);
  if (slash != NULL) {
    name = g_strndup(path, (gsize)(slash - path));
    next_path = slash + 1;
    if (*next_path == '\0') next_path = NULL;
  } else {
    name = g_strdup(path);
    next_path = NULL;
  }
 ...
 */
static void matemenu_tree_execute_moves(MateMenuTree *tree,
                                        MenuLayoutNode *layout,
...
static gboolean matemenu_tree_load_layout(MateMenuTree *tree, GError **error) {
  ...
  if (tree->layout) return TRUE;

  if (!matemenu_tree_canonicalize_path(tree, error)) return FALSE;

  menu_verbose("Loading menu layout from \"%s\"\n", tree->canonical_path);

  tree->layout = menu_layout_load(tree->canonical_path,
                                  tree->non_prefixed_basename, error);
  if (!tree->layout) return FALSE;

  loaded_menu_files = g_hash_table_new(g_str_hash, g_str_equal);
  g_hash_table_insert(loaded_menu_files, tree->canonical_path,
                      GUINT_TO_POINTER(TRUE));
...
static void matemenu_tree_force_reload(MateMenuTree *tree) {
  ...
  if (tree->layout) menu_layout_node_unref(tree->layout);
  tree->layout = NULL;
}
...
static void get_by_category_foreach(const char *file_id, DesktopEntry *entry,
...
#ifdef WITH_COLLECTION
static void get_by_desktop_foreach(const char *file_id, DesktopEntry *entry,
                                   GetByCategoryForeachData *data) {
...
static void get_by_category(DesktopEntrySet *entry_pool, DesktopEntrySet *set,
                            const char *category) {
  GetByCategoryForeachData data;
...
#ifdef WITH_COLLECTION
static void get_by_desktop(DesktopEntrySet *entry_pool, DesktopEntrySet *set,
  ...
  desktop_entry_set_foreach(
      entry_pool, (DesktopEntrySetForeachFunc)get_by_desktop_foreach, &data);
}
#endif /* WITH_COLLECTION */

static DesktopEntrySet *process_include_rules(MenuLayoutNode *layout,
  ...
  switch (menu_layout_node_get_type(layout)) {
    case MENU_LAYOUT_NODE_AND: {
      ...
      while (child != NULL) {
         ...
         * because it's an AND
         */
        if (desktop_entry_set_get_count(set) == 0) break;
    ...
    case MENU_LAYOUT_NODE_FILENAME: {
      ...
      if (entry != NULL) {
        set = desktop_entry_set_new();
        desktop_entry_set_add_entry(set, entry,
                                    menu_layout_node_get_content(layout));
      }
  ...
  if (set == NULL) set = desktop_entry_set_new(); /* create an empty set */
  ...
  while (iter != NULL) {
    switch (menu_layout_node_get_type(iter)) {
      ...
      case MENU_LAYOUT_NODE_MERGE:
        *layout_info =
            g_slist_prepend(*layout_info, menu_layout_node_ref(iter));
        break;
...
static void entries_listify_foreach(const char *desktop_file_id,
                                    DesktopEntry *desktop_entry,
                                    MateMenuTreeDirectory *directory) {
  directory->entries =
      g_slist_prepend(directory->entries,
                      matemenu_tree_entry_new(directory, desktop_entry,
                                              desktop_file_id, FALSE, FALSE));
}

static void excluded_entries_listify_foreach(const char *desktop_file_id,
                                             DesktopEntry *desktop_entry,
                                             MateMenuTreeDirectory *directory) {
  directory->entries =
      g_slist_prepend(directory->entries,
                      matemenu_tree_entry_new(directory, desktop_entry,
                                              desktop_file_id, TRUE, FALSE));
}

static void unallocated_entries_listify_foreach(
    const char *desktop_file_id, DesktopEntry *desktop_entry,
    MateMenuTreeDirectory *directory) {
  directory->entries =
      g_slist_prepend(directory->entries,
                      matemenu_tree_entry_new(directory, desktop_entry,
                                              desktop_file_id, FALSE, TRUE));
}

static void set_default_layout_values(MateMenuTreeDirectory *parent,
...
static MateMenuTreeDirectory *process_layout(MateMenuTree *tree,
                                             MateMenuTreeDirectory *parent,
                                             MenuLayoutNode *layout,
  ...
  directory = matemenu_tree_directory_new(
      tree, parent, menu_layout_node_menu_get_name(layout));
  ...
  entry_pool = _entry_directory_list_get_all_desktops(
      menu_layout_node_menu_get_app_dirs(layout));
    ...
    switch (menu_layout_node_get_type(layout_iter)) {
      case MENU_LAYOUT_NODE_MENU:
        /* recurse */
        {
          ...
          child_dir = process_layout(tree, directory, layout_iter, allocated);
          if (child_dir)
            directory->subdirs = g_slist_prepend(directory->subdirs, child_dir);
      ...
      case MENU_LAYOUT_NODE_DIRECTORY: {
         ...
         */
        entry = entry_directory_list_get_directory(
            menu_layout_node_menu_get_directory_dirs(layout),
            menu_layout_node_get_content(layout_iter));
        ...
        menu_verbose("Processed <Directory> new directory entry = %p (%s)\n",
                     ...
                     directory->directory_entry
                         ? desktop_entry_get_path(directory->directory_entry)
                         : "null");
      } break;
      ...
      case MENU_LAYOUT_NODE_DEFAULT_LAYOUT:
        menu_layout_node_default_layout_get_values(
            layout_iter, &directory->default_layout_values);
  ...
  if (directory->directory_entry) {
    ...
    if (!desktop_entry_get_show_in(directory->directory_entry)) {
      menu_verbose(
          "Not showing menu %s because OnlyShowIn!=$DESKTOP or "
          "NotShowIn=$DESKTOP (with $DESKTOP=${XDG_CURRENT_DESKTOP:-GNOME})\n",
          desktop_entry_get_name(directory->directory_entry));
      deleted = TRUE;
    }
  }
  ...
  if (deleted) {
    if (excluded_set != NULL) desktop_entry_set_unref(excluded_set);
  ...
  desktop_entry_set_foreach(
      entries, (DesktopEntrySetForeachFunc)entries_listify_foreach, directory);
  desktop_entry_set_unref(entries);
  ...
  while (tmp != NULL) {
    ...
    if (!desktop_entry_get_show_in(entry->desktop_entry)) {
      menu_verbose(
          "Deleting %s because OnlyShowIn!=$DESKTOP or NotShowIn=$DESKTOP "
          "(with $DESKTOP=${XDG_CURRENT_DESKTOP:-GNOME})\n",
          desktop_entry_get_name(entry->desktop_entry));
      delete = TRUE;
    }
...
static void process_only_unallocated(MateMenuTree *tree,
                                     MateMenuTreeDirectory *directory,
                                     DesktopEntrySet *allocated,
  ...
  if (directory->only_unallocated) {
    ...
    while (tmp != NULL) {
      ...
      if (desktop_entry_set_lookup(allocated, entry->desktop_file_id)) {
        ...
        desktop_entry_set_add_entry(unallocated_used, entry->desktop_entry,
...
static void get_still_unallocated_foreach(
    const char *file_id, DesktopEntry *entry,
    GetStillUnallocatedForeachData *data) {
  if (desktop_entry_set_lookup(data->allocated, file_id)) return;

  if (desktop_entry_set_lookup(data->unallocated_used, file_id)) return;

  /* Same rules than at the end of process_layout() */
  if (desktop_entry_get_hidden(entry)) return;

  if (!(data->tree->flags & MATEMENU_TREE_FLAGS_INCLUDE_NODISPLAY) &&
      desktop_entry_get_no_display(entry))
    return;

  if (!desktop_entry_get_show_in(entry)) return;
...
static GSList *get_layout_info(MateMenuTreeDirectory *directory,
...
static void get_values_with_defaults(MenuLayoutNode *node,
...
static guint get_real_subdirs_len(MateMenuTreeDirectory *directory) {
  guint len;
  GSList *tmp;
...
static void preprocess_layout_info_subdir_helper(
    MateMenuTree *tree, MateMenuTreeDirectory *directory,
    MateMenuTreeDirectory *subdir, MenuLayoutValues *layout_values,
    gboolean *contents_added, gboolean *should_remove) {
  ...
  *contents_added = FALSE;
  ...
  if (subdir->subdirs == NULL && subdir->entries == NULL) {
    ...
    if (layout_values->inline_alias &&
      ...
      menu_verbose(
          "Inline aliasing '%s' to '%s'\n",
          item->type == MATEMENU_TREE_ITEM_ENTRY
              ? g_app_info_get_name(G_APP_INFO(matemenu_tree_entry_get_app_info(
                    MATEMENU_TREE_ENTRY(item))))
              : (item->type == MATEMENU_TREE_ITEM_DIRECTORY
                     ? matemenu_tree_directory_get_name(
                           MATEMENU_TREE_DIRECTORY(item))
                     : matemenu_tree_directory_get_name(
                           MATEMENU_TREE_ALIAS(item)->directory)),
          subdir->name);
      ...
      *should_remove = TRUE;
             ...
             layout_values->inline_limit >=
                 real_subdirs_len + g_slist_length(subdir->entries)) {
      if (layout_values->inline_header) {
        ...
        g_slist_foreach(subdir->subdirs, (GFunc)matemenu_tree_item_set_parent,
                        directory);
        directory->subdirs =
            g_slist_concat(directory->subdirs, subdir->subdirs);
        subdir->subdirs = NULL;

        g_slist_foreach(subdir->entries, (GFunc)matemenu_tree_item_set_parent,
                        directory);
        directory->entries =
            g_slist_concat(directory->entries, subdir->entries);
        subdir->entries = NULL;
...
static void preprocess_layout_info(MateMenuTree *tree,
  ...
  while (tmp != NULL && last_subdir != NULL) {
    ...
    if (menu_layout_node_get_type(node) != MENU_LAYOUT_NODE_MENUNAME) continue;

    get_values_with_defaults(node, &layout_values,
    ...
    while (subdir_l != NULL) {
      ...
      if (!strcmp(subdir->name, name)) break;
    ...
    preprocess_layout_info_subdir_helper(tree, directory, subdir,
                                         &layout_values, &contents_added,
                                         &should_remove);
    ...
    if (should_remove) {
      if (last_subdir == subdir_l) {
        ...
        if (subdir_l == directory->subdirs)
          ...
          while (buf != NULL && buf->next != subdir_l) buf = buf->next;
          last_subdir = buf;
        }
      }
  ...
  /* remove the sentinel */
  directory->subdirs =
      g_slist_delete_link(directory->subdirs, directory->subdirs);
  ...
  if (strip_duplicates) {
    ...
    directory->entries = g_slist_sort(
        directory->entries, (GCompareFunc)matemenu_tree_entry_compare_by_id);
    tmp = directory->entries;
...
static void check_pending_separator(MateMenuTreeDirectory *directory) {
  if (directory->layout_pending_separator) {
    menu_verbose("Adding pending separator in '%s'\n", directory->name);

    directory->contents = g_slist_append(
        directory->contents, matemenu_tree_separator_new(directory));
    directory->layout_pending_separator = FALSE;
  }
}

static void merge_alias(MateMenuTree *tree, MateMenuTreeDirectory *directory,
                        MateMenuTreeAlias *alias) {
  ...
  directory->contents =
      g_slist_append(directory->contents, matemenu_tree_item_ref(alias));
}

static void merge_subdir(MateMenuTree *tree, MateMenuTreeDirectory *directory,
                         MateMenuTreeDirectory *subdir) {
  ...
  if (subdir->will_inline_header == 0 ||
    ...
    g_slist_foreach(subdir->contents, (GFunc)matemenu_tree_item_set_parent,
                    directory);
    ...
    directory->contents =
        g_slist_append(directory->contents, matemenu_tree_item_ref(subdir));
  }
}

static void merge_subdir_by_name(MateMenuTree *tree,
                                 MateMenuTreeDirectory *directory,
  ...
  while (tmp != NULL) {
    ...
    if (MATEMENU_TREE_ITEM(subdir)->type == MATEMENU_TREE_ITEM_ALIAS) continue;
...
static void merge_entry(MateMenuTree *tree, MateMenuTreeDirectory *directory,
                        MateMenuTreeEntry *entry) {
  ...
  check_pending_separator(directory);
  directory->contents =
      g_slist_append(directory->contents, matemenu_tree_item_ref(entry));
}

static void merge_entry_by_id(MateMenuTree *tree,
                              MateMenuTreeDirectory *directory,
                              const char *file_id) {
  GSList *tmp;
  ...
  while (tmp != NULL) {
    ...
    /* if it's an alias, then it cannot be affected by
     * the Merge nodes in the layout */
    if (MATEMENU_TREE_ITEM(entry)->type == MATEMENU_TREE_ITEM_ALIAS) continue;
...
static inline gboolean find_name_in_list(const char *name, GSList *list) {
  while (list != NULL) {
    if (!strcmp(name, list->data)) return TRUE;
...
static void merge_subdirs(MateMenuTree *tree, MateMenuTreeDirectory *directory,
                          GSList *except) {
  GSList *subdirs;
  GSList *tmp;
...
static void merge_entries(MateMenuTree *tree, MateMenuTreeDirectory *directory,
                          GSList *except) {
  GSList *entries;
  GSList *tmp;
...
static void merge_subdirs_and_entries(MateMenuTree *tree,
                                      MateMenuTreeDirectory *directory,
  ...
  while (tmp != NULL) {
    ...
    if (type == MATEMENU_TREE_ITEM_ALIAS) {
      ...
      if (!find_name_in_list(MATEMENU_TREE_DIRECTORY(item)->name,
                             except_subdirs)) {
        merge_subdir(tree, directory, MATEMENU_TREE_DIRECTORY(item));
        matemenu_tree_item_unref(item);
      } else {
        menu_verbose("Not merging directory '%s' yet\n",
      ...
      if (!find_name_in_list(MATEMENU_TREE_ENTRY(item)->desktop_file_id,
                             except_entries)) {
...
static GSList *get_subdirs_from_layout_info(GSList *layout_info) {
  GSList *subdirs;
  GSList *tmp;
  ...
  while (tmp != NULL) {
    ...
    if (menu_layout_node_get_type(node) == MENU_LAYOUT_NODE_MENUNAME) {
      subdirs =
          g_slist_append(subdirs, (char *)menu_layout_node_get_content(node));
    }
...
static GSList *get_entries_from_layout_info(GSList *layout_info) {
  GSList *entries;
  GSList *tmp;
  ...
  while (tmp != NULL) {
    ...
    if (menu_layout_node_get_type(node) == MENU_LAYOUT_NODE_FILENAME) {
      entries =
          g_slist_append(entries, (char *)menu_layout_node_get_content(node));
    }
  ...
  if (layout_info == NULL) {
    ...
    while (tmp != NULL) {
      ...
      switch (menu_layout_node_get_type(node)) {
        case MENU_LAYOUT_NODE_MENUNAME:
          merge_subdir_by_name(tree, directory,
                               menu_layout_node_get_content(node));
          break;

        case MENU_LAYOUT_NODE_FILENAME:
          merge_entry_by_id(tree, directory,
                            menu_layout_node_get_content(node));
          break;
        ...
        case MENU_LAYOUT_NODE_MERGE:
          switch (menu_layout_node_merge_get_type(node)) {
            ...
            case MENU_LAYOUT_MERGE_ALL:
              merge_subdirs_and_entries(
                  tree, directory, get_subdirs_from_layout_info(tmp->next),
                  get_entries_from_layout_info(tmp->next));
              break;
...
static void handle_entries_changed(MenuLayoutNode *layout, MateMenuTree *tree) {
...
static void update_entry_index(MateMenuTree *tree, MateMenuTreeDirectory *dir) {
  MateMenuTreeIter *iter = matemenu_tree_directory_iter(dir);
  MateMenuTreeItemType next_type;

  while ((next_type = matemenu_tree_iter_next(iter)) !=
    ...
    if (item != NULL) matemenu_tree_item_unref(item);
  }
...
static gboolean matemenu_tree_build_from_layout(MateMenuTree *tree,
  ...
  if (!matemenu_tree_load_layout(tree, error)) return FALSE;
  ...
  tree->root =
      process_layout(tree, NULL, find_menu_child(tree->layout), allocated);
  if (tree->root) {
    DesktopEntrySet *unallocated_used;
    ...
    if (tree->flags & MATEMENU_TREE_FLAGS_INCLUDE_UNALLOCATED) {
      ...
      entry_pool = _entry_directory_list_get_all_desktops(
          menu_layout_node_menu_get_app_dirs(find_menu_child(tree->layout)));
      ...
      desktop_entry_set_foreach(
          entry_pool, (DesktopEntrySetForeachFunc)get_still_unallocated_foreach,
          &data);
    ...
    menu_layout_node_root_add_entries_monitor(
        tree->layout, (MenuLayoutNodeEntriesChangedFunc)handle_entries_changed,
        tree);
  }
...
static void matemenu_tree_force_rebuild(MateMenuTree *tree) {
  if (tree->root) {
    ...
    menu_layout_node_root_remove_entries_monitor(
        tree->layout, (MenuLayoutNodeEntriesChangedFunc)handle_entries_changed,
        tree);
  }
}
...
GType matemenu_tree_iter_get_type(void) {
  ...
  if (gtype == G_TYPE_INVALID) {
    gtype = g_boxed_type_register_static(
        "MateMenuTreeIter", (GBoxedCopyFunc)matemenu_tree_iter_ref,
...
GType matemenu_tree_directory_get_type(void) {
  ...
  if (gtype == G_TYPE_INVALID) {
    gtype = g_boxed_type_register_static(
        "MateMenuTreeDirectory", (GBoxedCopyFunc)matemenu_tree_item_ref,
...
GType matemenu_tree_entry_get_type(void) {
  ...
  if (gtype == G_TYPE_INVALID) {
    gtype = g_boxed_type_register_static(
        "MateMenuTreeEntry", (GBoxedCopyFunc)matemenu_tree_item_ref,
        (GBoxedFreeFunc)matemenu_tree_item_unref);
  }
  return gtype;
}
...
GType matemenu_tree_separator_get_type(void) {
  ...
  if (gtype == G_TYPE_INVALID) {
    gtype = g_boxed_type_register_static(
        "MateMenuTreeSeparator", (GBoxedCopyFunc)matemenu_tree_item_ref,
        (GBoxedFreeFunc)matemenu_tree_item_unref);
  }
  return gtype;
}
...
GType matemenu_tree_header_get_type(void) {
  ...
  if (gtype == G_TYPE_INVALID) {
    gtype = g_boxed_type_register_static(
        "MateMenuTreeHeader", (GBoxedCopyFunc)matemenu_tree_item_ref,
        (GBoxedFreeFunc)matemenu_tree_item_unref);
  }
  return gtype;
}
...
GType matemenu_tree_alias_get_type(void) {
  ...
  if (gtype == G_TYPE_INVALID) {
    gtype = g_boxed_type_register_static(
        "MateMenuTreeAlias", (GBoxedCopyFunc)matemenu_tree_item_ref,
        (GBoxedFreeFunc)matemenu_tree_item_unref);
  }
  return gtype;
}
  ...
  if (G_UNLIKELY(!enum_type_id)) {
    static const GFlagsValue values[] = {
        ...
        {MATEMENU_TREE_FLAGS_INCLUDE_EXCLUDED,
         "MATEMENU_TREE_FLAGS_INCLUDE_EXCLUDED", "include-excluded"},
        {MATEMENU_TREE_FLAGS_SHOW_EMPTY, "MATEMENU_TREE_FLAGS_SHOW_EMPTY",
         "show-empty"},
        {MATEMENU_TREE_FLAGS_INCLUDE_NODISPLAY,
         "MATEMENU_TREE_FLAGS_INCLUDE_NODISPLAY", "include-nodisplay"},
        {MATEMENU_TREE_FLAGS_SHOW_ALL_SEPARATORS,
         "MATEMENU_TREE_FLAGS_SHOW_ALL_SEPARATORS", "show-all-separators"},
        {MATEMENU_TREE_FLAGS_SORT_DISPLAY_NAME,
         "MATEMENU_TREE_FLAGS_SORT_DISPLAY_NAME", "sort-display-name"},
        {MATEMENU_TREE_FLAGS_INCLUDE_UNALLOCATED,
         "MATEMENU_TREE_FLAGS_INCLUDE_UNALLOCATED,", "include-unallocated"},
        {0, NULL, NULL}};
