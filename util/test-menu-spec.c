...
static GOptionEntry options[] = {
    {"file", 'f', 0, G_OPTION_ARG_STRING, &menu_file, N_("Menu file"),
     N_("MENU_FILE")},
    {"monitor", 'm', 0, G_OPTION_ARG_NONE, &monitor,
     N_("Monitor for menu changes"), NULL},
    {"include-excluded", 'i', 0, G_OPTION_ARG_NONE, &include_excluded,
     N_("Include <Exclude>d entries"), NULL},
    {"include-nodisplay", 'n', 0, G_OPTION_ARG_NONE, &include_nodisplay,
...
static void print_entry(MateMenuTreeEntry *entry, const char *path) {
  ...
  utf8_path = g_filename_to_utf8(
      matemenu_tree_entry_get_desktop_file_path(entry), -1, NULL, NULL, NULL);

  utf8_file_id = g_filename_to_utf8(
      matemenu_tree_entry_get_desktop_file_id(entry), -1, NULL, NULL, NULL);
  ...
  while (TRUE) {
    ...
    switch (matemenu_tree_iter_next(iter)) {
      ...
      case MATEMENU_TREE_ITEM_ALIAS: {
        ...
        if (matemenu_tree_alias_get_aliased_item_type(item) ==
            MATEMENU_TREE_ITEM_ENTRY) {
          MateMenuTreeEntry *entry =
              matemenu_tree_alias_get_aliased_entry(item);
...
int main(int argc, char **argv) {
  ...
  options_context = g_option_context_new(
      _("- test MATE's implementation of the Desktop Menu Specification"));
  ...
  if (include_excluded) flags |= MATEMENU_TREE_FLAGS_INCLUDE_EXCLUDED;
  if (include_nodisplay) flags |= MATEMENU_TREE_FLAGS_INCLUDE_NODISPLAY;
  if (include_unallocated) flags |= MATEMENU_TREE_FLAGS_INCLUDE_UNALLOCATED;

  tree = matemenu_tree_new(menu_file ? menu_file : "mate-applications.menu",
                           flags);
  g_assert(tree != NULL);
